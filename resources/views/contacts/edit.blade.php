            ...
            <div class="bg-white shadow sm:rounded-lg p-6">
                <form action="{{ route('contacts.update', $contact) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input name="subject" value="{{ old('subject', $contact->subject) }}"
                            class="w-full border rounded px-3 py-2" />
                        @error('subject')
                            <p class="text-red-600 text-sm">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="message" rows="4" class="w-full border rounded px-3 py-2">{{ old('message', $contact->message) }}</textarea>
                        @error('message')
                            <p class="text-red-600 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
